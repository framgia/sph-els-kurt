...
const Lesson = () => {
  ...
  const renderAnswerButton = () => {
    if (answers.data.length !== lesson.data.words?.length) {
      ...
      return (
        <Link
          to={`/lessons/${lessonId}/result`}
          className="inline-flex items-center px-4 py-2 border border-gray-500 shadow-sm text-sm font-medium rounded-md text-gray-500 bg-white hover:bg-gray-50 "
        >
          View Result
        </Link>
      );
